...
TINY_NUMBER = 1e-6 
def img2mse_mask(x, y, mask=None):
    if mask is None:
        return torch.mean((x - y) * (x - y))
    else:
        return torch.sum((x - y) * (x - y) * mask) / (torch.sum(mask) * x.shape[-1] + TINY_NUMBER)
...
class NeRF(nn.Module):
    def __init__(self, D=8, W=256, input_ch=3, input_ch_views=3, input_ch_exps=1, output_ch=7, skips=[4], use_viewdirs=False, split_RGB=True):
        """ 
        """
    ...
    def forward(self, x):
        input_pts, input_views, input_exps, events = torch.split(x, [self.input_ch, self.input_ch_views, self.input_ch_exps, self.input_ch_events], dim=-1)
        h = input_pts
        ...
        if self.use_viewdirs:
            ...
            if self.split_RGB:
                ...
                r_h_s = rgb_h[:,0:1] + torch.log(input_exps) + events
                g_h_s = rgb_h[:,1:2] + torch.log(input_exps) + events
                b_h_s = rgb_h[:,2:3] + torch.log(input_exps) + events
                ...
                rgb_h_r = rgb_h[:,:,None]
                # print('input_exps', input_exps.shape)
                # print('rgb_h_r', rgb_h_r.shape)
                # print('events', events.shape)
                input_exps = input_exps[:,:,None].repeat(1,3,1)
                # print('input_exps', input_exps.shape)
                events = events[:,:,None].repeat(1,3,1)
                b_rgb_h_r = rgb_h_r + events
                h = b_rgb_h_r + torch.log(input_exps)
...
def point_constraint(model, gt, input_value):
    ...
    if model.split_RGB:
        ...
        for i, l in enumerate(model.exps_linears_r):
            ln_x = model.exps_linears_r[i](ln_x)
            ln_x = F.relu(ln_x)
        ln_x_l = model.rgb_l_linner(ln_x)   
        rgb_l = torch.sigmoid(torch.cat([ln_x_l, ln_x_l, ln_x_l], -1))
