...
def merge_event(event):
    return np.sum(event, axis=0)

def avg_event(event):
    return np.mean(event, axis=0)

def get_latent_image(event, bins, c_thred):
    zero_event = np.zeros_like(event[0:1])
    new_event = np.concatenate((zero_event, event), axis=0)
    latent_image = []
    ll = len(new_event)
    print('bins', bins)
    if bins == 1:
        for i in range(bins):
            a = merge_event(new_event) * c_thred
            latent_image.append(np.exp(a))
    elif bins == 3:
        for i in range(bins):
            a = merge_event(new_event[:2*(i) + 1]) * c_thred
            latent_image.append(np.exp(a))            
    elif bins == 5:
        for i in range(bins):
            a = merge_event(new_event[:i+1]) * c_thred
            latent_image.append(np.exp(a))
    elif bins == 9:
        new_new_event = separation_event(new_event)
        for i in range(bins):
            a = merge_event(new_new_event[:i+1]) * c_thred
            latent_image.append(np.exp(a))
    return np.array(latent_image)

def separation_event(event_list):
    new_event_list = [event_list[0]]
    for i in range(1, len(event_list)):
        sub_event = event_list[i]
        a = sub_event / 2.0
        a = np.round(a)
        b = sub_event - a
        new_event_list.append(a)
        new_event_list.append(b)
    return new_event_list
           
def get_blur_event(event, bins, c_thred):
    blur_event = []    
    for i in range(0, event.shape[0], 5):
        sub_event = event[i:i+4]
        latent_image = get_latent_image(sub_event, bins, c_thred)
        blur_image = avg_event(latent_image)
        for i in range(7):
            blur_event.append(blur_image)
    # the last one is not blur
    for i in range(7):
        non_zeros = np.ones_like(blur_event[-1])
        blur_event.append(non_zeros)
    blur_event = np.array(blur_event)
    blur_event = np.log(blur_event)
    print('blur_event', blur_event.shape)    
    return blur_event          

def load_syn_event_data(basedir, factor=8, recenter=True, bd_factor=.75, spherify=False, path_zflat=False, max_exp=1, min_exp=1, near_depth=4.0, render_size=30, c_thred=0.1, bins=5):
    ...
    event_poses = np.concatenate([event_poses[:, 0:1, :], 
                                  event_poses[:, 1:2, :], 
                                  event_poses[:, 2:, :]], 1)
    ...
    blur_event = get_blur_event(event, bins, c_thred)
    print('blur_event', blur_event.shape)
    print(a)
